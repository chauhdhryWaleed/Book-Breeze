...
const Navbar2 = ({ showNavbar }) => { // Accepting a prop to conditionally show/hide the Navbar
    const [isMenuOpen, setIsMenuOpen] = useState(false);
    const [isSticky, setIsSticky] = useState(false);
    const location = useLocation(); // Using useLocation hook to get the current route
    ...
    const navItems = [
        ...
        { link: "AddBook", path: "/addBook" },
        { link: "Contact Us", path: "/contactus" },
        { link: "My Profile", path: "/myprofile" },
        { link: "Cart", path: "/cart" },
        { link: "Sell your book ", path: "/admin/daishboard" }
    ]

    // Don't render Navbar if showNavbar is false or if the current route is "/login"
    if (!showNavbar || location.pathname === "/login") {
        return null;
    }
        ...
        <header className='w-full bg-transparent fixed top-0 left-0 right-0 transition-all duration-300'>
            <nav className={`py-4 lg:px-24 px-4 ${isSticky ? "sticky top-0 left-0 right-0 bg-blue-300" : ""}`}>
                <div className='flex justify-between items-center text-base gap-8 '>
                    {/* Logo */}
                    <Link to="/" className='text-2xl font-bold text-blue-700'><FaBlog className='inline-block' />BOOKS</Link>
                    ...
                    <div className='space-x-12 hidden lg:flex items-center'>
                        ...
                        {isMenuOpen && <button><FaBarsStaggered className='w-5 hover:text-blue-700' /></button>}
                    </div>
                ...
                <div className={`space-y-4 px-4 mt-16 py-7 bg-green-700 ${isMenuOpen ? "block fixed top-0 right-0 left-0" : "hidden"}`}>
